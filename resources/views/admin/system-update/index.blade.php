    ...
    @if($configured && $manifest)
        <div class="mt-8 bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
            ...
            @if($updateAvailable && $enabled)
                <form method="POST" action="{{ route('admin.system-update.apply') }}" class="mt-6 space-y-4" onsubmit="return confirm(@json(__('Run update now? The site may be briefly unavailable. A backup is recommended.')));">
                    @csrf
                    <label class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                        ...
                        <span>{{ __('I understand that config/ and .env stay untouched; uploads in storage/app/public and backups are preserved. I have a backup.') }}</span>
                    </label>
