...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit News') }}</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">{{ __('Edit news article') }}</p>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        @include('admin.news.partials.rich-text-editor', ['initialHtml' => old('content', $news->content)])
                        @error('content')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    @if($news->image)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Current Image') }}</label>
                            <img src="{{ asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="w-full h-48 object-cover rounded-lg">
                        </div>
                    @endif
                    
                    <!-- Image -->
                    <div>
                        <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ $news->image ? __('Replace Image') : __('Featured Image') }}</label>
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-dark-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-600 transition-colors">
                                <input type="radio" name="published" value="1" {{ old('published', $news->published) == '1' ? 'checked' : '' }} class="w-4 h-4 text-primary-500 focus:ring-primary-500">
                                <div>
                            ...
                            <label class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-dark-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-600 transition-colors">
                                <input type="radio" name="published" value="0" {{ old('published', $news->published) == '0' ? 'checked' : '' }} class="w-4 h-4 text-primary-500 focus:ring-primary-500">
                                <div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $news->publishedAtForDatetimeLocal()) }}"
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Leave empty to keep current time when Published. Future date = scheduled.') }}</p>
                        @error('published_at')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-dark-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-600 transition-colors">
                            <input type="checkbox" name="comments_enabled" value="1" @checked(old('comments_enabled', $news->commentsEnabled() ? '1' : '0') === '1') class="mt-0.5 w-4 h-4 text-primary-500 focus:ring-primary-500 rounded">
                            <div>
